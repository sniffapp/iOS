    ...
    */
    open var objects: Results<T> {
        ...
        if self.sectionNameKeyPath != nil {
            return self.fetchRequest.fetchObjects().filter(using: NSPredicate(format:"%K == %@", self.sectionNameKeyPath!,self.rbqFetchedResultsSectionInfo.name))
        }
