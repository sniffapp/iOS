...
static CGFloat const kMCStop1                       = 0.0; // Percentage limit to trigger the first action
static CGFloat const kMCStop2                       = 0.0; // Percentage limit to trigger the second action
...
- (void)setSwipeGestureWithView:(UIView *)view
    ...
    if ((state & MCSwipeTableViewCellState2) == MCSwipeTableViewCellState2) {
        ...
        if (view.frame.origin.x == 0)
            view.frame = CGRectIntegral(CGRectMake(view.frame.origin.x - 20, view.frame.origin.y, view.frame.size.width, view.frame.size.height));
        _view2 = view;
        _color2 = color;
        _modeForState2 = mode;
    }
